...
class TestFirstPageProgressChecker(TestProgressChecker):
    ...
    async def test_sets_headers(
        self, checker, add_response, httpx_mock: pytest_httpx.HTTPXMock, token
    ):
...
class TestLastPageProgressChecker(TestProgressChecker):
    ...
    async def add_page_response(self, httpx_mock: pytest_httpx.HTTPXMock):
        def adder(
            page_number,
            num_elements_or_content,
            etag,
            match_if_none_match,
            status_code,
        ):
            assert page_number > 0
            assert isinstance(num_elements_or_content, str) or (
                0 <= num_elements_or_content <= self.PER_PAGE
            )
    ...
    async def test_has_changes_if_exception(
        self, checker, add_page_response, httpx_mock: pytest_httpx.HTTPXMock
    ):
        add_page_response(1, self.PER_PAGE, '"tag1"', None, 200)
